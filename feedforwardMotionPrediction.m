% feedforwardMotionPrediction.m
% MATLAB Feedforward Neural Network for Motion Prediction
...
% Debugging: Print actual data sizes
fprintf('Loaded Input Size: %s\n', mat2str(size(inputs)));
fprintf('Loaded Output Size: %s\n', mat2str(size(outputs)));
...
% Debugging: Print sizes before training
fprintf('Final Size of X_ffn: %s\n', mat2str(size(X_ffn)));
fprintf('Final Size of y_ffn: %s\n', mat2str(size(y_ffn)));
...
figure;
subplot(2,1,1);
plot(y_ffn(1:1000,1), 'b', 'DisplayName', 'True');
hold on;
plot(predictedOutputs(1:1000,1), 'r', 'DisplayName', 'Predicted');
...
subplot(2,1,2);
plot(y_ffn(1:100,1), 'b', 'DisplayName', 'True');
hold on;
plot(predictedOutputs(1:100,1), 'r', 'DisplayName', 'Predicted');
