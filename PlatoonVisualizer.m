classdef PlatoonVisualizer < handle
    ...
    %
    % Author: zplotzke
    % Last Modified: 2025-02-09 21:48:54 UTC
    % Version: 1.5.8
    ...
    properties
        ...
        progress_panel % Handle to progress panel
        progress_bar  % Handle to progress bar background
        progress_fill % Handle to progress bar fill
        progress_text % Handle to progress text
    ...
    methods
        ...
        function visualize(obj, state)
            ...
            title('Truck Platoon Movement', 'FontSize', 12);
            ...
            % Export figure with UI components
            ...
            try
                ...
                % Use exportapp for better UI component support
                if exist('exportapp', 'file')
                    exportapp(obj.figure_handle, output_file);
                else
                    % Temporarily hide progress panel for saving
                    progress_visible = get(obj.progress_panel, 'Visible');
                    set(obj.progress_panel, 'Visible', 'off');

                    % Save without UI components
                    warning('off', 'MATLAB:ui:javaframe:PropertyToBeRemoved');
                    print(obj.figure_handle, '-dpng', '-r300', output_file);
                    warning('on', 'MATLAB:ui:javaframe:PropertyToBeRemoved');

                    % Restore progress panel visibility
                    set(obj.progress_panel, 'Visible', progress_visible);
                end
            catch export_error
                warning('Failed to export with UI components. Saving without progress bar.');
                % Hide progress panel
                set(obj.progress_panel, 'Visible', 'off');

                % Basic figure save
                saveas(obj.figure_handle, output_file);

                % Restore progress panel
                set(obj.progress_panel, 'Visible', 'on');
            end
        end
    ...
    methods (Access = private)
        ...
        function animateSimulation(obj, times, positions, velocities, accelerations, jerks, lengths)
            ...
            for frame = 1:num_frames
                ...
                % Update view window to follow trucks
                window_center = (min_pos + max_pos) / 2;
                xlim([window_center - window_width/2, window_center + window_width/2]);
